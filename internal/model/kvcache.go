...
// kv_gb = kv_per_token_gb * (context_tokens + generated_tokens - matched_prefix_tokens)
func KVCacheGB(r *Request, kvPerTokenGB float64) float64 {
	return kvPerTokenGB * float64(r.TotalTokens()-r.MatchedPrefixTokens)
}

// KVCacheMaxGB computes the worst-case KV-cache memory for a request
// (when all output tokens are generated).
// kv_gb = kv_per_token_gb * (context_tokens + output_tokens - matched_prefix_tokens)
func KVCacheMaxGB(r *Request, kvPerTokenGB float64) float64 {
	return kvPerTokenGB * float64(r.ContextTokens+r.OutputTokens-r.MatchedPrefixTokens)
}
